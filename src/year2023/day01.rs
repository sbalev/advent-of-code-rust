...
pub fn part1(input: Option<&str>) -> u32 {
    input.unwrap_or(INPUT).lines().map(line_value1).sum()
}

pub fn part2(input: Option<&str>) -> u32 {
    input.unwrap_or(INPUT).lines().map(line_value2).sum()
}
...
fn search_digit(line: &[u8], range: impl Iterator<Item = usize>) -> u8 {
    for i in range {
        ...
        for (j, d) in DIGIT_NAMES.iter().enumerate() {
...
";
        ...
        assert_eq!(281, part2(Some(example)));
    }
}
